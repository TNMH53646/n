...
local BC = loadstring(game:HttpGet("https://web-z992.onrender.com/ui-lib"))()

local _t, _o, _onch = {}, {
    TweenSpeed=160, TargetDistTP=0, IslandTPCD=1, TargetTPCD=0,
    M1Speed=0, Distance=12, KillAuraRange=200, KillAuraCD=0,
    WSValue=16, TPWValue=1, JPValue=50, HHValue=2,
    GravValue=196, ZoomValue=128, FOVValue=70, LimitFPSValue=60,
    OverrideTimeValue=12, AutoHopMins=30, RollCD=0,
    StatsRollCD=0, SpecRollCD=0, UpgradeLimit=0,
    Lock_MinSS=0, Del_MinSS=0, WebhookDelay=5,
    AutoSkill_BossHP=100, InstaKillHP=90, InstaKillMinHP="100000", SwitchWeaponCD=4,
    SelectedMob={}, SelectedBosses={}, AllMobType="Normal",
    SelectedWeaponType={}, SelectedKickType={"Mod"}, SelectedFarmType="Auto",
    SelectedMovementType="Tween", InstaKillType={}, ComboMode="Normal",
    SelectedStats={}, SelectedGemStats={}, SelectedTrait={},
    SelectedRace={}, SelectedClan={}, SelectedEnchant={},
    SelectedBlessing={}, SelectedPassive={}, SelectedSpec={},
    Lock_Type={}, Lock_Set={}, Lock_MS={}, Lock_SS={},
    Del_Type={}, Del_Set={}, Del_MS_Helmet={}, Del_MS_Gloves={},
    Del_MS_Body={}, Del_MS_Boots={}, Del_SS={},
    Up_MS={}, Eq_Type={}, Eq_MS={}, Eq_SS={},
    SelectedDungeon=nil, SelectedDiff=nil, MenuKeybind="U",
    WebhookURL="", UID="",
    SelectedData={}, SelectedItemRarity={"Common","Uncommon","Rare","Epic","Legendary","Mythical","Secret"},
    SelectedPriority_1="Boss", SelectedPriority_2="Pity Boss",
    SelectedPriority_3="Summon [Other]", SelectedPriority_4="Summon",
    SelectedPriority_5="Level Farm", SelectedPriority_6="All Mob Farm",
    SelectedPriority_7="Mob", SelectedPriority_8="Merchant",
    SelectedPriority_9="Alt Help",
}, {}

Toggles = setmetatable({},{__index=function(_,k)
    return {
        Value=_t[k] or false,
        SetValue=function(_,v) _t[k]=v if _onch[k] then _onch[k](v) end end,
        OnChanged=function(_,fn) _onch[k]=fn task.spawn(function() fn(_t[k] or false) end) end,
    }
end})

local _dropdowns = {} -- key → dropdown UI object (set after UI creation)

Options = setmetatable({},{__index=function(_,k)
    return {
        Value=_o[k],
        SetValues=function(_,list)
            if _dropdowns[k] then _dropdowns[k]:SetOptions(list) end
        end,
        SetValue=function(_,v) _o[k]=v if _dropdowns[k] then _dropdowns[k]:SetValue(nil,v) end end,
        OnChanged=function(_,fn) task.spawn(function() fn(_o[k]) end) end,
        OnClick=function() end,
    }
end})

Library = {
    Toggles=Toggles, Options=Options,
    ForceCheckbox=false, ShowToggleFrameInKeybinds=false,
    ShowCustomCursor=false, KeybindFrame={Visible=false},
    CornerRadius=6, Unloaded=false,
    Notify=function() end, SetNotifySide=function() end,
    SetDPIScale=function() end,
    Unload=function() Library.Unloaded=true end,
}
ThemeManager={SetLibrary=function()end,SetFolder=function()end,LoadDefault=function()end,ApplyToTab=function()end}
SaveManager={SetLibrary=function()end,SetFolder=function()end,IgnoreThemeSettings=function()end,
    SetIgnoreIndexes=function()end,BuildConfigSection=function()end,LoadAutoloadConfig=function()end}

getgenv().FourHub_Running = true

local function bcT(tab,id,title,def)
    _t[id]=def or false
    tab:Toggle({Title=title,Default=def or false,Callback=function(v)
        _t[id]=v if _onch[id] then _onch[id](v) end
    end})
end
local function bcS(tab,id,title,def,mn,mx)
    _o[id]=def
    tab:Slider({Title=title,Default=def,Min=mn,Max=mx,Callback=function(v) _o[id]=v end})
end
local function bcB(tab,title,cb) tab:Button({Title=title,Callback=cb}) end
local function bcSec(tab,t) tab:Section(t) end
...
local function PostToWebhook()
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = string.format("NixHub Ã¢â‚¬Â¢ Session: %s Ã¢â‚¬Â¢ %s", GetSessionTime(), os.date("%x %X")) },
...
local function UpdateAscendUI(data)
    if data.isMaxed then
        Tables.AscendLabels[1]:SetText("Ã¢Â­Â Max Ascension Reached!")
    ...
    for i = 1, 10 do
        ...
        if req then
            ...
            local status = req.completed and " Ã¢Å“â€¦" or " Ã¢ÂÅ’"
...
local function SendSafetyWebhook(targetPlayer, reason)
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = "NixHub Ã¢â‚¬Â¢ " .. os.date("%x %X") }
        }}
    }
...
-- BC UI Window
local _nullLabel = {SetText=function()end,SetVisible=function()end}
PityLabel = _nullLabel
TierLabel = _nullLabel
TimeLabel = _nullLabel
StatsLabel = _nullLabel
SpecPassiveLabel = _nullLabel
ArtifactLabel = _nullLabel
DustLabel = _nullLabel
InvLabel_Helmet = _nullLabel
InvLabel_Gloves = _nullLabel
InvLabel_Body = _nullLabel
InvLabel_Boots = _nullLabel
DungeonCount = _nullLabel
MerchantTimerLabel = _nullLabel
ValentineLabel = _nullLabel
ValentinePriceLabel = _nullLabel
for i = 1, 10 do Tables.AscendLabels[i] = _nullLabel end
if not Shared.SpecStatsSlider then Shared.SpecStatsSlider = {} end
setmetatable(Shared.SpecStatsSlider, {__index=function() return _nullLabel end})

local FPS_T = {Value=false}
local FPS_S = {Value=60, SetVisible=function()end}
local TPW_T = {Value=false}
local TPW_S = {Value=1, SetVisible=function()end}

local Win = BC:CreateWindow({Title="BlackCrown-X", Game="Sailor Piece", LogoId=77747658251236})

local TabFarm     = Win:Tab({Title="Farm"})
local TabBoss     = Win:Tab({Title="Boss"})
local TabCombat   = Win:Tab({Title="Combat"})
local TabAuto     = Win:Tab({Title="Auto"})
local TabArtifact = Win:Tab({Title="Artifact"})
local TabDungeon  = Win:Tab({Title="Dungeon"})
local TabPlayer   = Win:Tab({Title="Player"})
local TabTeleport = Win:Tab({Title="Teleport"})
local TabMisc     = Win:Tab({Title="Misc"})

-- Farm
bcSec(TabFarm,"Mob Farm")
bcT(TabFarm,"MobFarm","Autofarm Selected Mob",false)
bcT(TabFarm,"AllMobFarm","Autofarm All Mobs",false)
bcT(TabFarm,"LevelFarm","Autofarm Level",false)
bcT(TabFarm,"IslandTP","Island TP [Autofarm]",true)
bcT(TabFarm,"SwitchWeapon","Auto Switch Weapon",true)
do
    local dd = TabFarm:Dropdown({Title="Select Mob",Options=Tables.MobList,Default="",Callback=function(v)
        _o.SelectedMob={[v]=true}
    end})
    _dropdowns["SelectedMob"] = dd
end
bcSec(TabFarm,"Config")
bcS(TabFarm,"TweenSpeed","Tween Speed",160,0,500)
bcS(TabFarm,"Distance","Farm Distance",12,0,30)
bcS(TabFarm,"M1Speed","M1 Attack CD",0,0,100)
bcS(TabFarm,"IslandTPCD","Island TP CD",1,0,3)
bcS(TabFarm,"TargetTPCD","Target TP CD",0,0,5)
bcS(TabFarm,"TargetDistTP","Target Dist TP",0,0,100)
bcS(TabFarm,"SwitchWeaponCD","Switch Weapon CD",4,1,20)
bcSec(TabFarm,"Insta Kill")
bcT(TabFarm,"InstaKill","Instant Kill",false)
bcS(TabFarm,"InstaKillHP","HP% Insta-Kill",90,1,100)

-- Boss
bcSec(TabBoss,"Boss Farm")
bcT(TabBoss,"BossesFarm","Autofarm Selected Boss",false)
bcT(TabBoss,"AllBossesFarm","Autofarm All Bosses",false)
do
    local dd = TabBoss:Dropdown({Title="Select Boss",Options=Tables.BossList,Default="",Callback=function(v)
        _o.SelectedBosses={[v]=true}
    end})
    _dropdowns["SelectedBosses"] = dd
end
bcSec(TabBoss,"Summon")
bcT(TabBoss,"AutoSummon","Auto Summon",false)
bcT(TabBoss,"SummonBossFarm","Autofarm Summon Boss",false)
do
    local dd = TabBoss:Dropdown({Title="Select Summon",Options=Tables.SummonList,Default="",Callback=function(v)
        _o.SelectedSummon={[v]=true}
    end})
    _dropdowns["SelectedSummon"] = dd
end
bcSec(TabBoss,"Pity Boss")
bcT(TabBoss,"PityBossFarm","Autofarm Pity Boss",false)
bcSec(TabBoss,"Other Summon")
bcT(TabBoss,"AutoOtherSummon","Auto Summon (Other)",false)
bcT(TabBoss,"OtherSummonFarm","Autofarm Other Summon Boss",false)
bcSec(TabBoss,"Alt Help")
bcT(TabBoss,"AltBossFarm","Auto Help Alt",false)

-- Combat
bcSec(TabCombat,"Haki")
bcT(TabCombat,"ObserHaki","Auto Observation Haki",false)
bcT(TabCombat,"ArmHaki","Auto Armament Haki",false)
bcT(TabCombat,"ConquerorHaki","Auto Conqueror Haki",false)
bcSec(TabCombat,"Attack")
bcT(TabCombat,"AutoM1","Auto Attack",false)
bcT(TabCombat,"KillAura","Kill Aura",false)
bcS(TabCombat,"KillAuraCD","Kill Aura CD",0,0,100)
bcS(TabCombat,"KillAuraRange","Kill Aura Range",200,0,200)
bcSec(TabCombat,"Skills")
bcT(TabCombat,"AutoSkill_BossOnly","Boss Only",false)
bcS(TabCombat,"AutoSkill_BossHP","Boss HP%",100,1,100)
bcT(TabCombat,"OnlyTarget","Target Only",false)
bcT(TabCombat,"AutoSkill","Auto Use Skills",false)
bcSec(TabCombat,"Combo")
bcT(TabCombat,"ComboBossOnly","Boss Only",false)
bcT(TabCombat,"AutoCombo","Auto Skill Combo",false)
bcT(TabCombat,"AntiKnockback","Anti Knockback",false)

-- Auto
bcSec(TabAuto,"Ascend & Rolls")
bcT(TabAuto,"AutoAscend","Auto Ascend",false)
bcT(TabAuto,"AutoTrait","Auto Roll Trait",false)
bcT(TabAuto,"AutoRace","Auto Roll Race",false)
bcT(TabAuto,"AutoClan","Auto Roll Clan",false)
bcS(TabAuto,"RollCD","Roll Delay",0,0,100)
bcSec(TabAuto,"Stats & Gem")
bcT(TabAuto,"AutoStats","Auto UP Stats",false)
bcT(TabAuto,"AutoRollStats","Auto Roll Stats",false)
bcS(TabAuto,"StatsRollCD","Gem Roll Delay",0,0,100)
bcSec(TabAuto,"Enchant & Passive")
bcT(TabAuto,"AutoEnchant","Auto Enchant",false)
bcT(TabAuto,"AutoEnchantAll","Auto Enchant All",false)
bcT(TabAuto,"AutoBlessing","Auto Blessing",false)
bcT(TabAuto,"AutoBlessingAll","Auto Blessing All",false)
bcT(TabAuto,"AutoSpec","Auto Reroll Passive",false)
bcS(TabAuto,"SpecRollCD","Spec Roll Delay",0,0,100)
bcSec(TabAuto,"Skill & Milestone")
bcT(TabAuto,"AutoSkillTree","Auto Skill Tree",false)
bcT(TabAuto,"ArtifactMilestone","Auto Artifact Milestone",false)
bcSec(TabAuto,"Switch")
bcT(TabAuto,"AutoTitle","Auto Title",false)
bcT(TabAuto,"AutoRune","Auto Rune",false)
bcSec(TabAuto,"Trade")
bcT(TabAuto,"ReqTrade","Auto Send Request",false)
bcT(TabAuto,"ReqTradeAccept","Auto Accept Request",false)
bcT(TabAuto,"AutoAccept","Auto Accept Trade",false)

-- Artifact
bcSec(TabArtifact,"Lock")
bcS(TabArtifact,"Lock_MinSS","Min Sub-Stats",0,0,4)
bcT(TabArtifact,"ArtifactLock","Auto Lock",false)
bcSec(TabArtifact,"Delete")
bcS(TabArtifact,"Del_MinSS","Min Sub-Stats",0,0,4)
bcT(TabArtifact,"ArtifactDelete","Auto Delete",false)
bcT(TabArtifact,"DeleteUnlock","Auto Delete Unlocked",false)
bcSec(TabArtifact,"Upgrade")
bcS(TabArtifact,"UpgradeLimit","Upgrade Limit",0,0,15)
bcT(TabArtifact,"ArtifactUpgrade","Auto Upgrade",false)
bcT(TabArtifact,"UpgradeStage","Upgrade in Stages",false)
bcSec(TabArtifact,"Equip")
bcT(TabArtifact,"ArtifactEquip","Auto Equip",false)

-- Dungeon
bcSec(TabDungeon,"Dungeon")
bcT(TabDungeon,"AutoDungeon","Auto Join Dungeon",false)
bcT(TabDungeon,"AutoInfiniteTower","Auto Infinite Tower",false)
bcT(TabDungeon,"AutoDiff","Auto Select Difficulty",false)
bcT(TabDungeon,"AutoReplay","Auto Replay",false)
bcT(TabDungeon,"DungeonAutofarm","Autofarm Dungeon",false)

-- Player
bcSec(TabPlayer,"Movement")
bcT(TabPlayer,"WS","WalkSpeed",false)
bcS(TabPlayer,"WSValue","WalkSpeed Value",16,16,250)
bcT(TabPlayer,"JP","JumpPower",false)
bcS(TabPlayer,"JPValue","JumpPower Value",50,0,500)
bcT(TabPlayer,"HH","HipHeight",false)
bcS(TabPlayer,"HHValue","HipHeight Value",2,0,10)
bcT(TabPlayer,"Grav","Gravity",false)
bcS(TabPlayer,"GravValue","Gravity Value",196,0,500)
bcT(TabPlayer,"Zoom","Camera Zoom",false)
bcS(TabPlayer,"ZoomValue","Zoom Value",128,128,5000)
bcT(TabPlayer,"FOV","Field of View",false)
bcS(TabPlayer,"FOVValue","FOV Value",70,30,120)
bcT(TabPlayer,"Noclip","Noclip",false)
bcT(TabPlayer,"Disable3DRender","Disable 3D Rendering",false)
bcSec(TabPlayer,"Visual")
bcT(TabPlayer,"Fullbright","Fullbright",false)
bcT(TabPlayer,"NoFog","No Fog",false)
bcT(TabPlayer,"OverrideTime","Time Of Day",false)
bcS(TabPlayer,"OverrideTimeValue","Time Value",12,0,24)
bcT(TabPlayer,"FPSBoost","FPS Boost",false)
bcT(TabPlayer,"FPSBoost_AF","FPS Boost [Autofarm]",false)
bcT(TabPlayer,"LimitFPS","Limit FPS",false)
bcS(TabPlayer,"LimitFPSValue","Max FPS",60,5,360)
bcSec(TabPlayer,"Server")
bcT(TabPlayer,"AntiAFK","Anti AFK",true)
bcT(TabPlayer,"AntiKick","Anti Kick",false)
bcT(TabPlayer,"AutoReconnect","Auto Reconnect",false)
bcT(TabPlayer,"NoGameplayPaused","No Gameplay Paused",false)
bcT(TabPlayer,"AutoServerhop","Auto Serverhop",false)
bcS(TabPlayer,"AutoHopMins","Hop Every X Mins",30,0,300)
bcT(TabPlayer,"InstantPP","Instant Prompt",false)
bcT(TabPlayer,"AntiKnockback","Anti Knockback",false)
bcB(TabPlayer,"Serverhop",function()
    pcall(function()
        local Servers = game:HttpGet("https://games.roblox.com/v1/games/"..game.PlaceId.."/servers/Public?sortOrder=Asc&limit=100")
    end)
end)
bcB(TabPlayer,"Rejoin",function()
    Services.TeleportService:Teleport(game.PlaceId, Plr)
end)
bcB(TabPlayer,"Panic",function()
    PanicStop()
end)
bcB(TabPlayer,"Unload",function()
...
-- Teleport
bcSec(TabTeleport,"Islands")
bcB(TabTeleport,"Teleport to Level Quest",function()
...
bcSec(TabTeleport,"Puzzle")
bcB(TabTeleport,"Complete Dungeon Puzzle",function()
    local currentLevel = Plr.Data.Level.Value
    if currentLevel >= 5000 then UniversalPuzzleSolver("Dungeon")
    else Library:Notify("Level 5000 required! Current: "..currentLevel, 3) end
end)
bcB(TabTeleport,"Complete Slime Puzzle",function() UniversalPuzzleSolver("Slime") end)
bcB(TabTeleport,"Complete Valentine Puzzle",function()
    local Character = GetCharacter()
...
bcB(TabTeleport,"Complete Demonite Puzzle",function() UniversalPuzzleSolver("Demonite") end)
bcB(TabTeleport,"Complete Hogyoku Puzzle",function()
    local currentLevel = Plr.Data.Level.Value
    if currentLevel >= 8500 then UniversalPuzzleSolver("Hogyoku")
    else Library:Notify("Level 8500 required! Current: "..currentLevel, 4) end
end)

-- Misc
bcSec(TabMisc,"Merchant")
bcT(TabMisc,"AutoMerchant","Auto Buy Merchant",false)
bcT(TabMisc,"AutoDungeonMerchant","Auto Buy Dungeon",false)
bcT(TabMisc,"AutoValentineMerchant","Auto Buy Valentine",false)
bcT(TabMisc,"AutoTowerMerchant","Auto Buy Tower",false)
bcT(TabMisc,"AutoBossRushMerchant","Auto Buy Boss Rush",false)
bcSec(TabMisc,"Items")
bcT(TabMisc,"AutoChest","Auto Open Chest",false)
bcT(TabMisc,"AutoCraftItem","Auto Craft Item",false)
bcSec(TabMisc,"Quest")
bcT(TabMisc,"AutoQuestline","Auto Questline [BETA]",false)
bcSec(TabMisc,"Misc")
bcT(TabMisc,"AutoDeleteNotif","Auto Hide Notification",false)
bcT(TabMisc,"AutoKick","Auto Kick",true)
bcB(TabMisc,"Redeem All Codes",function()
        ...
        if playerLevel >= levelReq then
            ...
            if rewards then
                ...
                if rewards.Items and #rewards.Items > 0 then
                    local items = {}
                    for _, item in ipairs(rewards.Items) do table.insert(items, string.format("x%d %s", item.Quantity, item.Name)) end
...
bcSec(TabMisc,"Webhook")
bcT(TabMisc,"SendWebhook","Send Webhook",false)
bcT(TabMisc,"PingUser","Ping User",false)
bcB(TabMisc,"Test Webhook",function() PostToWebhook() end)

-- OnChanged callbacks
Toggles.SendWebhook:OnChanged(function(state) Thread("WebhookLoop", Func_WebhookLoop, state) end)
Toggles.LevelFarm:OnChanged(function(state) if not state then Shared.QuestNPC = "" end end)
Toggles.AutoTitle:OnChanged(function(state)
    if state and #Tables.UnlockedTitle == 0 then Remotes.TitleUnequip:FireServer() end
end)
Toggles.ObserHaki:OnChanged(function(state) Thread("AutoHaki", Func_AutoHaki, state) end)
Toggles.ArmHaki:OnChanged(function(state) Thread("AutoHaki", Func_AutoHaki, state) end)
Toggles.ConquerorHaki:OnChanged(function(state) Thread("AutoHaki", Func_AutoHaki, state) end)
Toggles.AutoM1:OnChanged(function(state) Thread("AutoM1", SafeLoop("Auto M1", Func_AutoM1), state) end)
Toggles.KillAura:OnChanged(function(state) Thread("KillAura", Func_KillAura, state) end)
Toggles.AutoSkill:OnChanged(function(state) Thread("AutoSkill", SafeLoop("Auto Skill", Func_AutoSkill), state) end)
Toggles.AutoStats:OnChanged(function(state) Thread("AutoStats", SafeLoop("Auto Stats", Func_AutoStats), state) end)
...
Toggles.AutoAscend:OnChanged(function(state)
    if state then Remotes.ReqAscend:InvokeServer()
    else
...
Toggles.AutoSpec:OnChanged(function(state) Thread("AutoSpecPassive", SafeLoop("Spec Passive", AutoSpecPassiveLoop), state) end)
Toggles.AutoRollStats:OnChanged(function(state) Thread("AutoRollStats", SafeLoop("Stat Roll", AutoRollStatsLoop), state) end)
Toggles.AutoSkillTree:OnChanged(function(state) Thread("AutoSkillTree", SafeLoop("Skill Tree", AutoSkillTreeLoop), state) end)
Toggles.ArtifactMilestone:OnChanged(function(state) Thread("ArtifactMilestone", Func_ArtifactMilestone, state) end)
...
Toggles.ArtifactLock:OnChanged(function(state) Thread("Artifact.Lock", SafeLoop("ArtifactLogic", Func_ArtifactAutomation), state) end)
Toggles.ArtifactDelete:OnChanged(function(state) Thread("Artifact.Delete", SafeLoop("ArtifactLogic", Func_ArtifactAutomation), state) end)
Toggles.ArtifactUpgrade:OnChanged(function(state) Thread("Artifact.Upgrade", SafeLoop("ArtifactLogic", Func_ArtifactAutomation), state) end)
Toggles.AutoDungeon:OnChanged(function(state) Thread("AutoDungeon", Func_AutoDungeon, state) end)
Toggles.AutoInfiniteTower:OnChanged(function(state) Thread("AutoInfiniteTower", Func_AutoInfiniteTower, state) end)
Toggles.AutoMerchant:OnChanged(function(state) Thread("AutoMerchant", SafeLoop("Merchant", Func_AutoMerchant), state) end)
Toggles.AutoChest:OnChanged(function(state) Thread("AutoChest", SafeLoop("Chest", Func_AutoChest), state) end)
Toggles.AutoCraftItem:OnChanged(function(state) Thread("AutoCraft", SafeLoop("Craft", Func_AutoCraft), state) end)
Toggles.AutoQuestline:OnChanged(function(state) Thread("AutoQuestline", SafeLoop("Questline", AutoQuestlineLoop), state) end)
...
_onch["LimitFPSValue"] = function()
    if _t.LimitFPS then setfpscap(_o.LimitFPSValue) end
end
...
Remotes.UpStatReroll.OnClientEvent:Connect(function(data)
    if data and data.Stats then Shared.GemStats = data.Stats task.spawn(UpdateStatsLabel) end
end)

Remotes.UpPlayerStats.OnClientEvent:Connect(function(data)
    if data and data.Stats then Shared.Stats = data.Stats UpdateStatsLabel() end
end)
        ...
        local success, err = pcall(function()
            ...
            if npcHum and npcRoot and root then
                ...
                if currentDist < 35 then
                    ...
                    if tick() - Shared.LastM1 >= m1Delay then
                        if Toggles.SwitchWeapon.Value then EquipWeapon() end
                        Remotes.M1:FireServer()
                        Shared.LastM1 = tick()
                    end
                end
